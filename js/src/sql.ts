...
export function buildSearchSql(config: Config, options: BuildOptions): BuiltQuery {
  ...
  if (highlight && haveText) {
    ...
    if (cfg.escapeHighlight) {
      // The delimiters are HTML, so the result is meant to be rendered, which makes the
      // document text around them active markup. Postgres does not escape it: its parser
      // drops tags it recognises, so <script> vanishes and it all looks safe, while
      // <img src=x onerror=...> and <svg/onload=...> come through whole. The ampersand has
      // to go first or the entities escape each other.
    ...
    outColumns.push(
      `ts_headline('${cfg.language}', ${document}, ` +
