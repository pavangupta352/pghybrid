...
import { forDrizzle, forKysely, forPg, forPostgresJs } from "../src/adapters.js";
...
const DOCUMENTS: [number, string, string][] = [
  ...
  [0.28, ANSWER, "Either party may terminate this agreement for convenience by giving sixty days written notice prior to the anniversary date. The notice period runs from the date of delivery."],
...
beforeAll(async () => {
  ...
  for (const [angle, title, content] of DOCUMENTS) {
    await pool.query(`INSERT INTO ${TABLE} (title, content, embedding) VALUES ($1, $2, $3)`, [
      title,
      content,
      unitVector(angle),
    ]);
...
describe.skipIf(!process.env.PGHYBRID_TEST_DSN && process.env.CI !== "true")(
  "adapters against a live server",
  () => {
    const expected = [ANSWER, "Renewal pricing", "Renewal terms"];

    it("node-postgres returns the fused ranking", async () => {
      if (!reachable) return;
    ...
    it("postgres.js agrees with node-postgres", async () => {
      if (!reachable) return;
    ...
    it("drizzle agrees, through its underlying client", async () => {
      if (!reachable) return;
    ...
    it("kysely agrees, through a raw compiled query", async () => {
      if (!reachable) return;
      ...
      const db = new Kysely<any>({ dialect: new PostgresDialect({ pool: ownPool }) });
      try {
    ...
    it("forces the numeric placeholder style whatever the config asked for", async () => {
      if (!reachable) return;
      ...
      const { sql } = search.buildQuery(QUERY, { embedding: EMBEDDING, limit: 3 });
